...
// Serve public directory from root-level "public"
...
app.post("/register-shop", (req, res) => {
  ...
  db.run(
    `INSERT INTO vendor (name, information, category, username, password) 
          VALUES (?, ?, ?, ?, ?)`,
  ...
  db.get("SELECT * FROM vendor WHERE id = ?", [vendorId], (err, vendor) => {
    if (err) {
      ...
      if (vendor) {
        ...
        db.all(
          "SELECT * FROM item WHERE vendor_id = ?",
          [vendorId],
          (err, items) => {
            if (err) {
              console.error(err.message);
...
app.post("/add-item", (req, res) => {
  ...
  const sql =
    "INSERT INTO item (name, price, image_url, description, vendor_id) VALUES (?, ?, ?, ?, ?)";
